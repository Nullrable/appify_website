...
export default function FeatureList({ app, lang }: FeatureListProps) {
  const appName = app.name[lang] || app.name["en"];
  const t = getSectionLabels(lang);
  const posts = listContentSlugs(app.id, "features", lang);
      ...
      <section className="relative overflow-hidden flex-1">
        ...
        <div className="relative max-w-3xl mx-auto px-6 py-16 md:py-24">
          <nav className="flex items-center gap-2 text-sm text-gray-500 mb-10">
            ...
            <span>/</span>
            <span className="text-gray-400">{t.features}</span>
          </nav>
          ...
          <header className="mb-12">
            ...
            <p className="text-gray-400 mt-3 text-lg">{t.featuresDesc}</p>
          </header>
          ...
          <ul className="space-y-6">
            {posts.map((post) => (
              <li key={post.slug}>
                <Link
                  to={`/${lang}/${app.id}/features/${post.slug}/`}
                ...
                >
                  <h2 className="font-heading font-semibold text-xl text-white transition-colors">
                    {post.title}
                  </h2>
